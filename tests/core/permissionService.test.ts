...
describe('PermissionService Unit Tests', () => {
  ...
  it('should preserve promoted system roles when clearAll uses default mode', async () => {
    const role = await service.createRole({
      name: 'Promoted Default Role',
      code: 'promoted_system_role_default',
      description: 'role promoted to system for default cleanup regression',
    ...
    try {
      ...
      expect(await service.getRoleById(role.id)).not.toBeNull();
    } finally {
      await service.clearAll(true);
    }
  });

  it('should remove promoted system roles when clearAll opts into full cleanup', async () => {
    const role = await service.createRole({
      name: 'Promoted Full Cleanup Role',
      code: 'promoted_system_role_full_cleanup',
      description: 'role promoted to system for full cleanup regression',
      permissionCodes: ['menu:view'],
    });
    ...
    expect(await service.getRoleById(role.id)).toBeNull();
  });
});
