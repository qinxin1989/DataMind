    ...
    if (urls.length > 0) {
      ...
      const runTask = async (url: string) => {
        try {
          ...
          const result = { url, selectors: { fields: {} }, error: e.message };
          results.push(result);
          onProgress?.({ type: 'result', content: result });
        }
      };
...
- 即使没有新网址，也要根据上下文（之前的 URL 和 HTML 采样）来思考。`;
  ...
  private cleanHtml(html: string): string {
    ...
    // 3. 极速降噪：保留核心 tag 和关键属性 (id, class, href)，移除其他所有属性
    ...
    // 4. 内容采样策略：如果依然过长，优先截取 HTML 的中间部分
    const limit = 40000;
...
4. 必须提取并返回department字段，从网页title、面包屑导航或URL路径中识别。`;
   ...
   * 获取用户模板列表
   */
  async getUserTemplates(userId: string): Promise<CrawlerTemplate[]> {
    ...
    const result: CrawlerTemplate[] = [];

    for (const template of templates) {
      const [fieldRows] = await pool.execute(
        `SELECT field_name, field_selector FROM crawler_template_fields WHERE template_id = ?`,
        [template.id]
      );
   ...
   * 获取单个模板
   */
  async getTemplateById(id: string, userId: string): Promise<CrawlerTemplate | null> {
    const [rows] = await pool.execute(
      `SELECT * FROM crawler_templates WHERE id = ? AND user_id = ?`,
      [id, userId]
    );
  ...
  async clearAll(): Promise<void> {
    await pool.execute('DELETE FROM crawler_template_fields');
