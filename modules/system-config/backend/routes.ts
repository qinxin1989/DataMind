...
export function initRoutes(db: any): Router {
   ...
   */
  router.post('/configs', requirePermission('system:config'), async (req: Request, res: Response) => {
    try {
   ...
   */
  router.delete('/configs/:key', requirePermission('system:config'), async (req: Request, res: Response) => {
    try {
