...
## Plan E：数据采集中心接入万表归一项目化流程（进行中）

### 背景
用户希望将 `D:\project\JW` 中的“万表归一”能力改造成 DataMind 的正式模块，并直接挂到“数据采集中心”。新的采集流程不再是孤立的文件处理，而是完整的项目化流水线：

1. 先维护标准表，可通过模型批量识别表头建立标准字段
2. 采集前先建项目，所有上传文件必须归属于项目
3. 批量导入或拖拽文件后，先做分类，再按每类自动脱敏一个样本
4. 脱敏能力要能单独使用，且输出文件格式与源文件保持一致
5. AI 测试数据也要能单独生成，且与源文件格式保持一致
6. 用脱敏文件或测试数据驱动模型生成 Python 采集脚本模板
7. 用 Python 脚本采集真实数据，再将模板表头与标准表头自动比对，完全一致自动映射，其余进入人工确认

### 当前拆解
- [x] Phase 1 — 梳理 JW 中标准表、分类、文件分析、批量处理和 Python 辅助脚本能力
- [x] Phase 2 — 对齐 DataMind 的数据采集菜单、模块清单、动态前端路由与鉴权桥接模式
- [x] Phase 3 — 新建 `universal-table` 模块骨架，并接入数据采集中心菜单与前后端入口
- [x] Phase 4 — 实现标准表、项目、项目文件、分类、脱敏、测试数据、采集脚本和映射结果的数据模型与后端接口
- [x] Phase 5 — 实现前端工作台，把“建标准表 → 建项目 → 上传分类 → 按类处理样本 → 生成/运行脚本 → 人工确认映射”串成一条流程
- [x] Phase 6 — 增加测试并完成后端/前端构建验证
- [x] Phase 7 — 采集结果自动入库，按项目+标准表生成受限分析数据源，供 Agent 直接挂业务技能分析
