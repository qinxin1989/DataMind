...
class ApprovalService {
  ...
  // 获取资源的最新申请
  async getLatestByResource(resourceId: string, type: ApprovalType): Promise<ApprovalRecord | null> {
    const [rows] = await pool.execute(
      'SELECT * FROM approval_records WHERE resource_id=? AND type=? ORDER BY created_at DESC LIMIT 1',
      [resourceId, type]
    );
    const row = (rows as any[])[0];
    return row ? this.mapRow(row) : null;
